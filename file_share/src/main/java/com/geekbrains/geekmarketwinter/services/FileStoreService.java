...
public class FileStoreService implements IFileStoreService {
    ...
    public void deleteFile(String filename) throws IOException {
        fileMetaProvider.deleteFileMeta(filename);
        UUID hash = fileMetaProvider.getMetaFilesByName(filename).getHash();
        if(fileMetaProvider.getMetaFilesByName(filename) == null) {
            systemProvider.deleteFile(hash.toString());
        }
    }
}
