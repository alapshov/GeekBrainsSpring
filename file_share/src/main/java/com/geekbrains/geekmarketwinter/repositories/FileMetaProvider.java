...
public class FileMetaProvider implements IFileMetaProvider {
    ...
    private static final String GET_FILES_META_BY_NAME = "select filename from vma.file_info_metadata where hash = :name";

    private static final String DELETE_FILE_PATH_BY_FILENAME = "DELETE from vma.file_info_metadata where hash = :filename";
    ...
    @Override
    public FileMetaDTO getMetaFilesByName(String filename) {
        try (Connection connection = sql2o.open()) {
            return connection.createQuery(GET_FILES_META_BY_NAME, false)
                    .addParameter("filename", filename)
                    .executeAndFetch(FileMetaDTO.class).stream().findFirst().get();
        }
    }
    ...
    public void deleteFileMeta(String filename) {
        try (Connection connection = sql2o.open()) {
            connection.createQuery(DELETE_FILE_PATH_BY_FILENAME)
                    .addParameter("filename", filename)
                    .executeUpdate();
        }
    }
